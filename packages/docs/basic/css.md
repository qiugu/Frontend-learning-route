...
从中也不难看出：
1. 横向布局时，外边距不会发生折叠
2. 包含一个`auto`值时，用整个容纳块的宽度减去设置的宽度，剩下的宽度分配给设置给`auto`的元素，因为外边距可以是负值，因此对于负值而言，也同样适用上述规则，可以看到第四个div就是负值外边距的情况。
3. 包含两个`auto`值时，就是将剩余距离平分为两份，每个`auto`值各占一半，这也是我们平时经常使用`margin: 0 auto;`居中的原理是一样的。
